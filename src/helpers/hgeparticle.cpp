...
hgeParticleSystem::hgeParticleSystem(const char *filename, hgeSprite *sprite)
{
	...
	DWORD size	= 0;
	psi=hge->Resource_Load(filename, &size);
	
	if(!psi) return;
	void *dst = &info;
	
	if (4 == sizeof(hgeParticleSystemInfo *)) memcpy(dst, psi, sizeof(hgeParticleSystemInfo));
		else if (8 == sizeof (hgeParticleSystemInfo*)) memcpy((void *) ((unsigned long )dst+4), psi, sizeof(hgeParticleSystemInfo)-4);
